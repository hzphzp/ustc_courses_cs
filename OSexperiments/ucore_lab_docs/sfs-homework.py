...
class fs:
    ...
    # YOUR CODE, 2013011359,2013011377
    ...
    # DONE
        inum = self.nameToInum[self.getParent(tfile)]
    ...
    # YOUR CODE, 2013011359,2013011377
        ...
        if self.data[pDnum].getFreeEntries() == 0:
        	return -1
        # if the newfile was already in parent dir?
        if self.data[pDnum].dirEntryExists(newfile):
        	return -1
        ...
        self.data[pDnum].addDirEntry(newfile, tInum)
    # DONE
        return tInum
    ...
    # YOUR CODE, 2013011359,2013011377
        ...
        pInum = self.nameToInum[parent]
        # is there room in the parent directory?
        if self.data[self.inodes[pInum].getAddr()].getFreeEntries() == 0:
        	return -1
        # have to make sure file name is unique
        if self.data[self.inodes[pInum].getAddr()].dirEntryExists(newfile):
        	return -1
        # find free inode
        nInum = self.inodeAlloc()
        if nInum < 0:
        	return -1
        self.inodes[nInum].setType(ftype)
        # if a directory, have to allocate directory block for basic (., ..) info
        if ftype == 'd':
        	nDnum = self.dataAlloc()
        	self.data[nDnum].setType(ftype)
        	self.inodes[nInum].setAll(ftype, nDnum, 1)
        	self.data[nDnum].addDirEntry('.', nInum)
        	self.data[nDnum].addDirEntry('..', pInum)
        	self.inodes[nInum].incRefCnt()
        # now ok to init inode properly
        # inc parent ref count
        self.inodes[pInum].incRefCnt()

        # and add to directory of parent
        self.data[self.inodes[pInum].getAddr()].addDirEntry(newfile, nInum)
    # DONE
        return nInum
