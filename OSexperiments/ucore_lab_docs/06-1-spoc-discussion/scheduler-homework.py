...
parser.add_option("-j", "--jobs", default=10, help="number of jobs in the system",
...
if options.jlist == '':
    for jobnum in range(0,options.jobs):
        ...
        IOtype = int(options.maxlen * random.random()) % 2
        joblist.append([jobnum, runtime, IOtype])
        print '  Job', jobnum, '( length = ' + str(runtime) +" iotype = "+ str(IOtype) +' )'
else:
    jobnum = 0
    for runtime in options.jlist.split(','):
        ...
        jobnum += 1
    for job in joblist:
        print '  Job', job[0], '( length = ' + str(job[1]) +" iotype = "+str(job[2])+' )'
print '\n'
...
if options.solve == True:
    ...
    if options.policy == 'RR':
        ...
        while jobcount > 0:
            ...
            iolen = len(iolist)
            runlen = len(runlist)
            if iolen > 0 and random.random() > 0.2:
                job = iolist.pop(0)
            else:
                job = runlist.pop(0)
            ...
            if runtime > quantum:
                ...
                if newIOtype == 1:
                    iolist.append([jobnum, runtime - ranfor, newIOtype])
                else :
                    runlist.append([jobnum, runtime - ranfor, newIOtype])
            else:
                #YOUR CODE
                ranfor = runtime
